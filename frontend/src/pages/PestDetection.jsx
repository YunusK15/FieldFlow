    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <span className="text-2xl shrink-0">{getPestIcon(pest.name)}</span>
          <h3 className="text-lg font-bold capitalize text-emerald-800 tracking-wide">{pest.name}</h3>
        </div>
        <svg className="chevron w-5 h-5 text-emerald-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
      ...
      <div className="pest-details">
        <div className="mb-3">
          ...
          <p className="text-sm leading-relaxed text-gray-700">{pest.description}</p>
        </div>
        <div className="rounded-xl p-3.5" style={{ background: 'var(--accent-light)', border: '1px solid rgba(22,101,52,0.1)' }}>
          <h4 className="text-xs font-semibold uppercase tracking-widest text-emerald-700 mb-1">💡 Solution</h4>
          <p className="text-sm leading-relaxed text-emerald-900">{pest.solution}</p>
        </div>
      </div>
    </div>
  )
      ...
      <section className="max-w-6xl mx-auto px-4 py-12 md:py-16">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <form onSubmit={handleUpload} className="space-y-5">
              ...
              >
                ...
                {preview ? (
                  ...
                  <div className="py-6">
                    <div className="mx-auto w-16 h-16 rounded-2xl bg-emerald-100 flex items-center justify-center mb-4 border border-emerald-200">
          ...
          <div className="glass-card p-6 md:p-8 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
            <h2 className="text-xl font-bold text-gray-900 mb-5 flex items-center gap-2">
              <svg className="w-5 h-5 text-emerald-700" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              Prediction Result
            </h2>
            {prediction ? (
              <div className="space-y-6 animate-fade-in-up">
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-widest text-emerald-600 mb-1">Identified As</p>
                    <h3 className="text-3xl font-extrabold capitalize text-gray-900">{prediction.label}</h3>
                    <p className="text-sm text-gray-500 mt-1">Confidence score</p>
                  </div>
                </div>
                <hr className="section-divider" />
                <div>
                  <h4 className="text-xs font-semibold uppercase tracking-widest text-emerald-600 mb-2">Description</h4>
                  <p className="text-gray-700 leading-relaxed">{prediction.description}</p>
                </div>
                <div className="rounded-xl p-5" style={{ background: 'var(--accent-light)', border: '1px solid rgba(22,101,52,0.1)' }}>
                  <h4 className="text-xs font-semibold uppercase tracking-widest text-emerald-700 mb-2 flex items-center gap-1.5">💡 Recommended Solution</h4>
                  <p className="text-emerald-900 leading-relaxed">{prediction.solution}</p>
                </div>
              </div>
            ) : (
              <div className="flex flex-col items-center justify-center text-center py-16 text-gray-500">
                <div className="w-20 h-20 rounded-full bg-emerald-100 flex items-center justify-center mb-5 border border-emerald-200">
                ...
                <p className="text-gray-600 font-medium">Upload an image to see results</p>
                <p className="text-sm text-gray-500 mt-1">Our AI model will identify the pest and suggest a solution</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="mt-16 md:mt-20">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h2 className="text-2xl md:text-3xl font-extrabold text-gray-900 tracking-tight">Pest Directory</h2>
              <p className="text-gray-600 mt-1 text-sm">Click any card to view details & solutions</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {pests.length > 0 ? pests.map((pest, idx) => (
              ...
              <div className="col-span-full text-center py-16 glass-card">
                <div className="loader mx-auto mb-4" style={{ borderColor: 'rgba(22,101,52,0.15)', borderTopColor: 'var(--accent)' }} />
                <p className="text-gray-500">Loading pest directory...</p>
              </div>
            )}
          </div>
        </div>
