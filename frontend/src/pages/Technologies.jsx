      ...
      <section className="max-w-6xl mx-auto px-4 py-16 md:py-24">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-gray-400 mt-4">Loading technologies directory...</p>
          </div>
        ) : error ? (
          <div className="max-w-md mx-auto p-6 glass-card border-red-500/30 text-center">
            <p className="text-red-400 font-bold mb-2">⚠️ Failed to Load Directory</p>
            ...
            {technologies.map((tech, i) => (
              <div key={tech._id || i} className="glass-card p-7 md:p-8 animate-fade-in-up group" style={{ animationDelay: `${i * 0.08}s` }}>
                <div className="flex items-start gap-4 mb-5">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-white line-clamp-1">{tech.title}</h3>
                    <p className="text-xs text-emerald-400 font-medium mt-0.5">{tech.tagline}</p>
                  </div>
                </div>
                <p className="text-gray-400 text-sm leading-relaxed mb-5 line-clamp-3" title={tech.description}>{tech.description}</p>
                <div className="rounded-xl p-4" style={{ background: 'rgba(52,211,153,0.04)', border: '1px solid rgba(52,211,153,0.1)' }}>
                  ...
                  <ul className="space-y-2">
                    {tech.benefits?.map((b, j) => (
                      <li key={j} className="flex items-start gap-2 text-sm text-gray-300 line-clamp-2">
